                ...
                for p in para:
                    try:
                        ...
                        if zh in sentences:
                            ...
                            p = p.replace('"_ ', '"')
                            """chin_punct = u'CJK_SYMBOLS_AND_PUNCTUATION'
                            ru_p = re.findall(r'<se lang="ru">([0-9A-zŌÒǘāǍūÀǒÉěǚÈĀōáéǎóèǜàĚòÁúǑìíǖÓùĒīüǔēǐ\s\n/[.,\/#!$%\^&\*;:{}=\-_`~()]/]+?)</se>',p)
                            ru_part = ru_p[0]
                            zh_p = re.findall(r'<se lang="zh">([0-9A-zŌÒǘāǍūÀǒÉěǚÈĀōáéǎóèǜàĚòÁúǑìíǖÓùĒīüǔēǐ\s\n/[.,\/#!$%\^&\*;:{}=\-_`~()]/]+?)</se>',p)
                            zh1_part = zh_p[0]"""
                            s = re.sub(r'<para>','', re.sub(r'<se lang="ru".+</se>', "", p)).strip() #zh_p
                            sent = []
                            for sec in s.split("</w>"):
                                ...
                                for se in range(len(sec)):
                                    ...
                                    for t in reg:
                                        secT = sec[se]
                                        #test0 = re.compile(t[0])
                                        #test1 = re.compile(t[1])
                                        #secT = secT.replace(test0.search(secT),test1.search(secT))
